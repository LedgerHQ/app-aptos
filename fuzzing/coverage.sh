...
"$BUILDDIR/fuzz_tx_parser" "$CORPUSDIR"/*
llvm-profdata merge --sparse "$LLVM_PROFILE_FILE" -o "$BUILDDIR/fuzz_tx_parser.profdata"
llvm-cov show --object "$BUILDDIR/fuzz_tx_parser" --object "$BUILDDIR/libtxparser.so.1" -instr-profile="$BUILDDIR/fuzz_tx_parser.profdata" -show-line-counts-or-regions -output-dir="$HTMLCOVDIR" -format=html
llvm-cov report --object "$BUILDDIR/fuzz_tx_parser" --object "$BUILDDIR/libtxparser.so.1" -instr-profile="$BUILDDIR/fuzz_tx_parser.profdata"

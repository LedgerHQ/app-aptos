...
int ui_prepare_entry_function() {
    ...
    switch (function->known_type) {
        ...
        case FUNC_WITHDRAW_STAKE:
            if (is_coin_type_aptos(&function->args.delegation.ty_args)) {
                return ui_display_delegation_pool_transfer(function->known_type);
            } else {
                // If the coin type is not AptosCoin, we fall back to blindsigning
                // the transaction, as an entry function.
                memset(g_tx_type, 0, sizeof(g_tx_type));
                snprintf(g_tx_type, sizeof(g_tx_type), "Delegation pool");
                PRINTF("Tx Type: %s\n", g_tx_type);
                return UI_PREPARED;
            }
        default:
